...
import com.lucky.luckyojbackendjudgeservice.judge.codesandbox.CodeSandboxProxy;
...
import com.lucky.luckyojbackendserviceclient.service.QuestionFeignClient;
...
public class JudgeServiceImpl implements JudgeService {
    ...
    public QuestionSubmit doJudge(long questionSubmitId) {
        ...
        // 注意：这里需要你的 Question 实体类中存在对应的字段
        // 由于是远程调用，我们先查出原数据再更新（或者在 Feign 接口提供专门的自增接口）
        int newSubmitNum = (question.getSubmitNum() == null ? 0 : question.getSubmitNum()) + 1;
        updateQuestion.setSubmitNum(newSubmitNum);

        // 如果判题结果是 Accepted，通过数 +1
        if ("Accepted".equals(judgeInfo.getMessage())) {
            int newAcceptedNum = (question.getAcceptedNum() == null ? 0 : question.getAcceptedNum()) + 1;
            updateQuestion.setAcceptedNum(newAcceptedNum);
        }

        // 调用 Feign 更新题目信息
        boolean updateQuestionRes = questionFeignClient.updateQuestionById(updateQuestion);
        if (!updateQuestionRes) {
            log.error("更新题目统计数据失败, questionId: {}", questionId);
        }
