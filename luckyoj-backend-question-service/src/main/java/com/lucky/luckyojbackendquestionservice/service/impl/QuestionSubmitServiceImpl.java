...
//        });
    ...
    public SubmitSummaryVo getSubmitSummary(User loginUser) {
        ...
        // 1. 获取各难度题目总数（从题目表 Question 获取）
        List<Question> allQuestions = questionMapper.selectList(new LambdaQueryWrapper<Question>()
                .select(Question::getId, Question::getDifficulty));

        Map<String, List<Long>> difficultyIdsMap = allQuestions.stream()
                .collect(Collectors.groupingBy(Question::getDifficulty,
                        Collectors.mapping(Question::getId, Collectors.toList())));

        List<Long> easyIds = difficultyIdsMap.getOrDefault(ProblemDifficultyEnum.EASY.getValue(), new ArrayList<>());
        List<Long> mediumIds = difficultyIdsMap.getOrDefault(ProblemDifficultyEnum.MEDIUM.getValue(), new ArrayList<>());
        List<Long> hardIds = difficultyIdsMap.getOrDefault(ProblemDifficultyEnum.HARD.getValue(), new ArrayList<>());

        summaryVo.setEasyTotal(easyIds.size());
        summaryVo.setMediumTotal(mediumIds.size());
        summaryVo.setHardTotal(hardIds.size());
        summaryVo.setTotal(allQuestions.size());

        // 2. 获取用户【真正通过】的提交记录（必须是 Accepted 且状态为成功）
        List<QuestionSubmit> allPassSubmits = baseMapper.selectList(new LambdaQueryWrapper<QuestionSubmit>()
        ...
        // 3. 核心统计逻辑：按题号去重，计算各难度通过的【题目数量】
        // 拿到所有通过的题目 ID 集合（去重）
        Set<Long> passedQuestionIds = allPassSubmits.stream()
                .map(QuestionSubmit::getQuestionId)
                .collect(Collectors.toSet());

        // 计算分难度通过数（交集统计）
        int easyPass = (int) passedQuestionIds.stream().filter(easyIds::contains).count();
        int mediumPass = (int) passedQuestionIds.stream().filter(mediumIds::contains).count();
        int hardPass = (int) passedQuestionIds.stream().filter(hardIds::contains).count();

        summaryVo.setEasyPass(easyPass);
        summaryVo.setMediumPass(mediumPass);
        summaryVo.setHardPass(hardPass);
        summaryVo.setPassCount(passedQuestionIds.size());

        // 4. 获取用户总提交次数（不带 Accepted 过滤，纯次数统计）
        Long submitCount = baseMapper.selectCount(new LambdaQueryWrapper<QuestionSubmit>()
                .eq(QuestionSubmit::getUserId, userId));
        summaryVo.setSubmitCount(Math.toIntExact(submitCount));

        log.info("【统计结果】总题目: {}, 总提交: {}, 总通过(去重): {}",
                summaryVo.getTotal(), summaryVo.getSubmitCount(), summaryVo.getPassCount());
        log.info("【难度分布】简单通过: {}/{}，中等通过: {}/{}，困难通过: {}/{}",
                easyPass, easyIds.size(), mediumPass, mediumIds.size(), hardPass, hardIds.size());
        log.info("========== 统计结束 ==========");
